    ...
    module BuildUiAppTag
      ...
      def build_data(cache_keys = {}, current_user = nil, current_ability = nil)
        ...
        { version: Motor::VERSION,
          ...
          i18n: i18n_data,
          primary_color: style_primary_color(configs_cache_key),
          small_logo_url: small_logo_url(configs_cache_key),
      ...
      def style_primary_color(cache_key = nil)
        configs = Motor::Configs::LoadFromCache.load_configs(cache_key: cache_key)

        configs.find { |c| c.key == 'style.primary_color' }&.value || '#408ccf'
      end

      def small_logo_url(cache_key = nil)
        configs = Motor::Configs::LoadFromCache.load_configs(cache_key: cache_key)

        configs.find { |c| c.key == 'style.small_logo_url' }&.value
      end
